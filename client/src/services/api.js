...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response?.status === 500) {
      ...
      console.error('[API] Erro 500 do backend:', msg, error.config?.url ? `(${error.config.url})` : '');
    }
...
export const getCiclosIgreja = (igrejaId) => api.get(`/igrejas/${igrejaId}/ciclos`);
export const getCicloItens = (igrejaId, numeroCiclo) => api.get(`/igrejas/${igrejaId}/ciclos/${numeroCiclo}`);
export const saveCicloItens = (igrejaId, numeroCiclo, itens) =>
  api.put(`/igrejas/${igrejaId}/ciclos/${numeroCiclo}`, { itens });
export const getOrganistasParaCiclos = (igrejaId) => api.get(`/igrejas/${igrejaId}/ciclos-organistas`);
