...
// Migração: tabela ciclo_itens (Gestão de Ciclos - N ciclos por igreja)
const migrateCicloItens = async () => {
  try {
    const pool = getDb();
    const [tables] = await pool.execute(
      `SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES 
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'ciclo_itens'`
    );
    if (tables.length > 0) {
      return;
    }
    ...
    `);
    console.log('✅ Tabela ciclo_itens criada (Gestão de Ciclos).');
  } catch (error) {
